...
| Consumer smoke | `pnpm verify:consumer` (`scripts/consumer-smoke.js`): `npm pack`s both built dist folders into real tarballs, installs them into a throwaway fixture (`scripts/consumer-smoke/fixture/`, outside the pnpm workspace) as a real consumer would, and `tsc --noEmit`s a file importing only the bare `@quartz-headless/core`/`@quartz-headless/primitives` specifiers — catches `exports`-map/peerDependency/missing-dist-file problems `verify-build.js` can't (it only inspects the dist folders directly, never a real install). Run after `pnpm build:lib`; not part of pre-commit (real network install, slower). |
